...
interface SettingsSessionUi {
  activeSection: SettingsSection
  scrollTopBySection: Partial<Record<SettingsSection, number>>
}
...
export default function SettingsPage() {
  ...
  const remembered = uiStore.read<SettingsSessionUi>(SETTINGS_SESSION_KEY)
  ...
  const [activeSection, setActiveSection] = useState<SettingsSection>(remembered?.activeSection ?? 'sending')
  const [scrollTopBySection, setScrollTopBySection] = useState<Partial<Record<SettingsSection, number>>>(() => ({ ...(remembered?.scrollTopBySection || {}) }))
  ...
  useLayoutEffect(() => {
    ...
    if (!content) return
    content.scrollTop = Math.max(0, Number(scrollTopBySection[activeSection] || 0))
    // Restore only when entering another settings section or remounting the page.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [activeSection])

  useEffect(() => {
    uiStore.write<SettingsSessionUi>(SETTINGS_SESSION_KEY, { activeSection, scrollTopBySection })
  }, [uiStore, activeSection, scrollTopBySection])
  ...
  const selectSection = (section: SettingsSection) => {
    ...
    const currentTop = contentRef.current?.scrollTop || 0
    setScrollTopBySection(previous => ({ ...previous, [activeSection]: currentTop }))
    setActiveSection(section)
  }
  const rememberSectionScroll = (scrollTop: number) => {
    setScrollTopBySection(previous => previous[activeSection] === scrollTop ? previous : { ...previous, [activeSection]: scrollTop })
  }
  const savedUiScalePercent = scalePercent(values.ui_scale)

  return <div className="page page--settings"><PageHeader title="Настройки" description="Постоянные параметры работы — не нужно задавать их при каждом запуске" actions={<Button loading={save.isPending} onClick={() => save.mutate()}><Save size={17} />Сохранить изменения</Button>} />
    <div className="settings-layout"><SettingsTabs active={activeSection} onSelect={selectSection} /><div ref={contentRef} className="settings-content settings-content--bounded" onScroll={event => rememberSectionScroll(event.currentTarget.scrollTop)}>
