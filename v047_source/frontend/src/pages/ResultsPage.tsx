...
interface ResultsSessionUi {
  selectedRunId: number | null
  search: string
  selectedIds: number[]
  mode: 'table' | 'links'
  sort: 'completed_at' | 'group_name' | 'account'
  ascending: boolean
  columnWidths: number[]
  scrollTop: number
}

const DEFAULT_COLUMN_WIDTHS = [38, 300, 120, 120, 210, 145, 150]

function rememberedColumnWidths(value: number[] | undefined): number[] {
  if (!Array.isArray(value) || value.length !== DEFAULT_COLUMN_WIDTHS.length || value.some(item => !Number.isFinite(item) || item <= 0)) return DEFAULT_COLUMN_WIDTHS
  return [...value]
}
...
export default function ResultsPage({ kind }: { kind: 'success' | 'failed' }) {
  ...
  const sessionKey = `results.${kind}`
  const remembered = uiStore.read<ResultsSessionUi>(sessionKey)
  ...
  const [mode, setMode] = useState<'table' | 'links'>(remembered?.mode || 'table')
  const [sort, setSort] = useState<'completed_at' | 'group_name' | 'account'>(remembered?.sort || 'completed_at')
  const [ascending, setAscending] = useState(remembered?.ascending ?? false)
  const [historyId, setHistoryId] = useState<number | null>(null)
  const [columnWidths, setColumnWidths] = useState(() => rememberedColumnWidths(remembered?.columnWidths))
  const [scrollTop, setScrollTop] = useState(remembered?.scrollTop ?? 0)
  const parentRef = useRef<HTMLDivElement>(null)

  const runs = useQuery({ queryKey: ['runs'], queryFn: () => api<RunHistoryPayload>('/runs'), refetchInterval: 4000 })
  const runIds = useMemo(() => runs.data?.items.map(run => run.id) || [], [runs.data?.items])
  const { selectedRunId, selectRun, returnToCurrent } = useResultRunSelection(runs.data?.current_run_id ?? null, runIds, runs.data != null, sessionKey)
  const previousRunRef = useRef(selectedRunId)
  const query = useQuery({ queryKey: ['results', kind, selectedRunId, search], queryFn: () => api<{ total: number; items: ResultItem[] }>(`/results/${kind}?run_id=${selectedRunId}&search=${encodeURIComponent(search)}&limit=2000`), enabled: selectedRunId != null, refetchInterval: selectedRunId != null && selectedRunId === runs.data?.current_run_id ? 4000 : false })
  ...
  const removeRun = useMutation({ mutationFn: (runId: number) => api<{ ok: boolean }>(`/runs/${runId}`, { method: 'DELETE' }), onSuccess: (_result, deletedRunId) => { if (deletedRunId === selectedRunId) returnToCurrent(); client.invalidateQueries({ queryKey: ['runs'] }); client.invalidateQueries({ queryKey: ['results', 'success'] }); client.invalidateQueries({ queryKey: ['results', 'failed'] }); toast.success(`Запуск #${deletedRunId} удалён`) }, onError: (error: Error) => toast.error(error.message || 'Не удалось удалить запуск') })

  useEffect(() => {
    if (previousRunRef.current === selectedRunId) return
    previousRunRef.current = selectedRunId
    setSelected(new Set())
    setScrollTop(0)
  }, [selectedRunId])
  ...
  useEffect(() => {
    ...
    const existing = new Set(query.data.items.map(item => item.id))
  ...
  useEffect(() => {
    if (mode !== 'table' || !parentRef.current || !query.data) return
    parentRef.current.scrollTop = scrollTop
    // Restore only when the virtual body is recreated/refreshed, not on every scroll event.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [mode, selectedRunId, query.data])

  useEffect(() => {
    uiStore.write<ResultsSessionUi>(sessionKey, { selectedRunId, search, selectedIds: [...selected], mode, sort, ascending, columnWidths, scrollTop })
  }, [uiStore, sessionKey, selectedRunId, search, selected, mode, sort, ascending, columnWidths, scrollTop])
  ...
  function beginResize(index: number, event: ReactMouseEvent) {
    event.preventDefault(); const startX = event.clientX; const startWidth = columnWidths[index]
  ...
  async function copyLinks(rows: ResultItem[]) { await navigator.clipboard.writeText(rows.map(item => item.url).join('\n')); toast.success(`Скопировано ссылок: ${rows.length}`) }
  ...
  return <div className="page page--results">
    ...
    <Card className="table-card"><div className="table-toolbar"><label className="search-box"><Search size={17} /><input value={search} onChange={e => setSearch(e.target.value)} placeholder="Поиск по группе, ссылке или аккаунту" /></label><div className="toolbar-actions"><select value={sort} onChange={e => setSort(e.target.value as typeof sort)}><option value="completed_at">По времени</option><option value="group_name">По группе</option><option value="account">По аккаунту</option></select><Button variant="ghost" onClick={() => setAscending(value => !value)}>{ascending ? 'А–Я' : 'Я–А'}</Button><Button variant="secondary" onClick={() => copyTable(selected.size ? selectedRows : items)}><Clipboard size={16} />{selected.size ? 'Копировать выбранное' : 'Копировать таблицу'}</Button><Button variant="secondary" onClick={() => copyLinks(selected.size ? selectedRows : items)}><Link2 size={16} />{selected.size ? 'Выбранные ссылки' : 'Все ссылки'}</Button><Button variant="secondary" disabled={selectedRunId == null} onClick={() => selectedRunId != null && downloadExport(kind, 'links', exportSelection, selectedRunId)}><Download size={16} />TXT</Button><Button variant="secondary" disabled={selectedRunId == null} onClick={() => selectedRunId != null && downloadExport(kind, 'csv', exportSelection, selectedRunId)}><Download size={16} />Экспорт CSV</Button><Button variant="secondary" disabled={selectedRunId == null} onClick={() => selectedRunId != null && downloadExport(kind, 'xlsx', exportSelection, selectedRunId)}><Download size={16} />XLSX</Button></div></div>
      {!items.length ? <EmptyState icon={success ? <CheckCircle2 /> : <CircleX />} title="Здесь пока пусто" text={selectedRunId == null ? 'Сначала создайте запуск — его результаты будут храниться отдельно.' : 'Для выбранного запуска в этом разделе пока нет результатов.'} /> : mode === 'links' ? <div className="links-mode"><div className="links-mode-head"><span>{items.length} ссылок</span><Button onClick={() => copyLinks(items)}><Clipboard size={16} />Скопировать всё</Button></div>{items.map(item => <a key={item.id} href={item.url} target="_blank">{item.url}<ExternalLink size={14} /></a>)}</div> : <>
        ...
        <div className="virtual-body" ref={parentRef} onScroll={event => setScrollTop(event.currentTarget.scrollTop)}><div style={{ height: `${virtual.getTotalSize()}px`, position: 'relative', minWidth: `${columnWidths.reduce((sum, value) => sum + value, 0) + 100}px` }}>{virtual.getVirtualItems().map(row => { const item = items[row.index]; const reason = [item.message_reason, item.suggested_reason].filter(Boolean).join('; '); return <div className="virtual-row result-grid" key={item.id} style={{ height: `${row.size}px`, transform: `translateY(${row.start}px)`, gridTemplateColumns: gridColumns }}><div><input type="checkbox" checked={selected.has(item.id)} onChange={e => setSelected(previous => { const next = new Set(previous); e.target.checked ? next.add(item.id) : next.delete(item.id); return next })} /></div><div><button className="link-button" onClick={() => setHistoryId(item.work_item_id)}>{item.group_name}</button><a href={item.url} target="_blank">{item.url}</a></div><div className={item.message_state === 'sent' ? 'yes' : 'no'}>{item.message_state === 'sent' ? '✓ Отправлено' : '× Не отправлено'}</div><div className={item.suggested_state === 'sent' ? 'yes' : 'no'}>{item.suggested_state === 'sent' ? '✓ Отправлено' : '× Не отправлено'}</div><div title={reason}>{success ? item.destination : reason || 'Неизвестная причина'}</div><div>{item.account}</div><div>{item.completed_at ? formatLocalDateTime(item.completed_at) : '—'}</div></div> })}</div></div></>}
    </Card>
    {historyId && <ScaledViewportOverlay><div className="drawer-backdrop" onMouseDown={() => setHistoryId(null)}><aside className="history-drawer" onMouseDown={event => event.stopPropagation()}><header><div><span>История обработки</span><h2>{history.data?.group_name || 'Загрузка…'}</h2></div><button className="icon-button" onClick={() => setHistoryId(null)}><X size={18} /></button></header>{history.data && <div className="history-content"><a href={history.data.url} target="_blank">{history.data.url}</a><div className="history-summary"><span>Аккаунт <b>{history.data.account || '—'}</b></span><span>Попыток <b>{history.data.attempts_count}</b></span><span>Итог <b>{history.data.result?.destination || history.data.state}</b></span></div><h3>ЛС сообщества</h3><p>{history.data.result?.message_reason || history.data.result?.message_state || 'Не выполнялось'}</p><h3>Предложенная запись</h3><p>{history.data.result?.suggested_reason || history.data.result?.suggested_state || 'Не выполнялось'}</p><h3>Попытки и события</h3>{history.data.attempts.map(attempt => <article key={attempt.id}><time>{formatLocalDateTime(attempt.created_at)}</time><strong>{attempt.direction === 'message' ? 'ЛС' : 'Предложка'} · {attempt.state}</strong><p>{attempt.reason || `VK object ID: ${attempt.vk_object_id || '—'}`}</p></article>)}{history.data.events.map(event => <article key={`event-${event.id}`}><time>{formatLocalDateTime(event.created_at)}</time><strong>{event.message}</strong>{Object.keys(event.technical).length > 0 && <pre>{JSON.stringify(event.technical, null, 2)}</pre>}</article>)}</div>}</aside></div></ScaledViewportOverlay>}
  </div>
}
