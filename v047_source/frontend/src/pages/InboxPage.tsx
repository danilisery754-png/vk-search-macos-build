...
import { ChevronDown, GripVertical, Inbox, MessageCircle, Paperclip, Pin, Plus, RefreshCcw, Search, Send, Smile, UserRound, X, Zap } from 'lucide-react'
...
import ScaledViewportOverlay from '../components/ScaledViewportOverlay'
...
import { INBOX_ACCOUNT_ORDER_STORAGE_KEY, moveInboxAccountOrder, normalizeInboxAccountOrder } from '../utils/inboxAccountOrder'
...
interface InboxAccountGroup { accountId: number; accountName: string; rows: Dialog[] }
type InboxFilter = 'all' | 'unread' | 'archive'
interface InboxSessionUi {
  filter: InboxFilter
  folderId: number | ''
  accountId: number | ''
  search: string
  ...
  folderSearch: string
  folderOrder: number[]
  draftsByDialogId: Record<string, string>
  replyTargetIdsByDialogId: Record<string, number>
}

const INBOX_SESSION_KEY = 'inbox'

function explicitFilter(search: string): InboxFilter | null {
  const value = new URLSearchParams(search).get('filter')
  return value === 'all' || value === 'unread' || value === 'archive' ? value : null
}

function readInboxAccountOrder(): number[] {
  try {
    const value = JSON.parse(localStorage.getItem(INBOX_ACCOUNT_ORDER_STORAGE_KEY) || '[]')
...
export default function InboxPage() {
  ...
  const remembered = uiStore.read<InboxSessionUi>(INBOX_SESSION_KEY)
  const requestedFilter = explicitFilter(location.search)
  const [filter, setFilter] = useState<InboxFilter>(requestedFilter ?? remembered?.filter ?? 'all')
  const [folderId, setFolderId] = useState<number | ''>(remembered?.folderId ?? '')
  const [accountId, setAccountId] = useState<number | ''>(remembered?.accountId ?? '')
  const [search, setSearch] = useState(remembered?.search ?? '')
  const [selected, setSelected] = useState<number | null>(remembered?.selectedDialogId ?? null)
  const [draftsByDialogId, setDraftsByDialogId] = useState<Record<string, string>>(() => ({ ...(remembered?.draftsByDialogId || {}) }))
  const [replyTargetIdsByDialogId, setReplyTargetIdsByDialogId] = useState<Record<string, number>>(() => ({ ...(remembered?.replyTargetIdsByDialogId || {}) }))
  const [collapsedAccountIds, setCollapsedAccountIds] = useState<number[]>(remembered?.collapsedAccountIds ?? [])
  const collapsed = useMemo(() => new Set(collapsedAccountIds), [collapsedAccountIds])
  const [inboxAccountOrder, setInboxAccountOrder] = useState<number[]>(readInboxAccountOrder)
  ...
  const [foldersCollapsed, setFoldersCollapsed] = useState(remembered?.foldersCollapsed ?? false)
  const [folderSearch, setFolderSearch] = useState(remembered?.folderSearch ?? '')
  ...
  const [folderOrder, setFolderOrder] = useState<number[]>(remembered?.folderOrder ?? [])
  ...
  const previousAccountIdRef = useRef<number | ''>(accountId)
  const reply = selected == null ? '' : draftsByDialogId[String(selected)] || ''
  const replyTargetId = selected == null ? null : replyTargetIdsByDialogId[String(selected)] ?? null
  const replyTarget = useMemo(() => replyTargetId == null ? null : renderedMessages.find(message => message.vk_message_id === replyTargetId) || null, [renderedMessages, replyTargetId])
  const setReply = useCallback<Dispatch<SetStateAction<string>>>((nextValue) => {
    if (selected == null) return
    setDraftsByDialogId(previous => {
  ...
  const setReplyTargetId = useCallback((value: number | null) => {
    if (selected == null) return
    setReplyTargetIdsByDialogId(previous => {
      const key = String(selected)
      if (value == null) {
  ...
  useEffect(() => {
    const routeFilter = explicitFilter(location.search)
  ...
  useEffect(() => {
    uiStore.write<InboxSessionUi>(INBOX_SESSION_KEY, {
      filter,
      folderId,
      accountId,
      search,
      selectedDialogId: selected,
      collapsedAccountIds,
      foldersCollapsed,
      folderSearch,
      folderOrder,
      draftsByDialogId,
      replyTargetIdsByDialogId,
    })
  }, [uiStore, filter, folderId, accountId, search, selected, collapsedAccountIds, foldersCollapsed, folderSearch, folderOrder, draftsByDialogId, replyTargetIdsByDialogId])
  useEffect(() => {
    if (!accounts.data) return
    const currentIds = accounts.data.map(account => account.id)
    setInboxAccountOrder(previous => {
      const next = normalizeInboxAccountOrder(previous, currentIds)
      if (sameNumberArray(previous, next)) return previous
      localStorage.setItem(INBOX_ACCOUNT_ORDER_STORAGE_KEY, JSON.stringify(next))
      return next
    })
    setCollapsedAccountIds(previous => previous.filter(id => currentIds.includes(id)))
  }, [accounts.data])
  ...
  const send = useMutation({
    mutationFn: async (input: { dialogId?: number | null; text?: string; replyTo?: number; forward?: Record<string, unknown> } = {}) => {
      const dialogId = input.dialogId === undefined ? selected : input.dialogId
      const text = input.text === undefined ? reply : input.text
      const replyTo = Object.prototype.hasOwnProperty.call(input, 'replyTo') ? input.replyTo : replyTargetId ?? undefined
      if (dialogId == null) throw new Error('Диалог не выбран')
      const result = await api<ReplyResult>(`/inbox/dialogs/${dialogId}/reply`, { method: 'POST', body: JSON.stringify({ body: text, reply_to: replyTo, forward: input.forward }) })
      if (!result.ok) throw new Error(result.error?.trim() || 'Не удалось отправить сообщение через VK')
      return { ...result, dialogId }
    },
    onSuccess: result => {
      setDraftsByDialogId(previous => {
        const key = String(result.dialogId)
        if (!(key in previous)) return previous
        const next = { ...previous }
        delete next[key]
        return next
      ...
      setForwardTarget(null); if (result.dialogId === selected && selected != null) syncDialog.mutate({ dialogId: selected, offset: 0, count: 50 }); client.invalidateQueries({ queryKey: ['dialogs'] })
    },
    onError: (error: Error) => toast.error(error.message),
  })
  ...
  const grouped = useMemo(() => {
    ...
    const naturalRank = new Map((accounts.data || []).map((account, index) => [account.id, index]))
    const savedRank = new Map(inboxAccountOrder.map((id, index) => [id, index]))
    const map = new Map<number, InboxAccountGroup>()

    for (const dialog of dialogs.data || []) {
      let group = map.get(dialog.account_id)
      if (!group) {
        group = {
          accountId: dialog.account_id,
          accountName: accountNames.get(dialog.account_id) || dialog.account_name || `Аккаунт ${dialog.account_id}`,
          rows: [],
        }
        map.set(dialog.account_id, group)
      }
      group.rows.push(dialog)
    }

    for (const group of map.values()) {
      group.rows.sort((a, b) => Number(Boolean(b.is_pinned)) - Number(Boolean(a.is_pinned)) || String(b.pinned_at || b.last_message_at || '').localeCompare(String(a.pinned_at || a.last_message_at || '')))
    }

    return [...map.values()].sort((left, right) => {
      const leftSaved = savedRank.get(left.accountId)
      const rightSaved = savedRank.get(right.accountId)
      if (leftSaved != null || rightSaved != null) return (leftSaved ?? Number.MAX_SAFE_INTEGER) - (rightSaved ?? Number.MAX_SAFE_INTEGER)
      const leftNatural = naturalRank.get(left.accountId)
      const rightNatural = naturalRank.get(right.accountId)
      return (leftNatural ?? Number.MAX_SAFE_INTEGER) - (rightNatural ?? Number.MAX_SAFE_INTEGER) || left.accountId - right.accountId
    })
  }, [accounts.data, dialogs.data, inboxAccountOrder])
  ...
  const dropAccount = (targetId: number) => {
    if (draggingAccountId == null || draggingAccountId === targetId) { setDraggingAccountId(null); setDragOverAccountId(null); return }
    const currentIds = (accounts.data || []).map(account => account.id)
    setInboxAccountOrder(previous => {
      const normalized = normalizeInboxAccountOrder(previous, currentIds)
      const next = moveInboxAccountOrder(normalized, draggingAccountId, targetId)
      localStorage.setItem(INBOX_ACCOUNT_ORDER_STORAGE_KEY, JSON.stringify(next))
      return next
    })
  ...
  useEffect(() => {
    if (selected == null) return
    setRenderedMessages([]); renderedRef.current = []; setBeforeCursor(null); setHasOlderLocal(false); setRemoteOffset(0); setHasMoreRemote(true); setSyncError(''); setShowNewMessages(false); isNearBottomRef.current = true; initialSyncRef.current = null; initializedDialogRef.current = null; setMessageMenu(null); setForwardTarget(null)
  ...
  return <div className="page page--inbox" onClick={() => { if (dialogMenu) setDialogMenu(null); if (messageMenu) setMessageMenu(null) }}>
      ...
      <section className="chat-panel">
        {selected == null ? <EmptyState icon={<MessageCircle />} title="Выберите диалог" text="Слева находятся переписки, сгруппированные по аккаунтам." /> : <>
          ...
          {canWrite ? <form className="composer" onSubmit={e => { e.preventDefault(); if (reply.trim()) send.mutate({}) }}>
            {replyTargetId != null && <div className="composer-reply"><span>Ответ на: {replyTarget?.body || `сообщение #${replyTargetId}`}</span><button type="button" aria-label="Отменить ответ" onClick={() => setReplyTargetId(null)}><X size={13} /></button></div>}
            <textarea ref={textareaRef} className="composer-input" aria-label="Ответ" value={reply} onChange={e => handleTyping(e.target.value)} placeholder="Напишите ответ…" rows={1} onKeyDown={e => { if (e.key === 'Escape') { setReplyTargetId(null); setActivePopover('closed') } else if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); if (reply.trim()) send.mutate({}) } }} />
    ...
    {dialogMenu && <ScaledOverlay point={{ x: dialogMenu.x, y: dialogMenu.y }}><div className="context-menu" onClick={event => event.stopPropagation()}>{filter === 'archive' || dialogMenu.value.is_archived ? <button onClick={() => archiveDialog.mutate({ id: dialogMenu.value.id, restore: true })}>Вернуть из архива</button> : <><button onClick={() => archiveDialog.mutate({ id: dialogMenu.value.id, restore: false })}>В архив</button><button onClick={() => pinDialog.mutate({ id: dialogMenu.value.id, is_pinned: !dialogMenu.value.is_pinned })}>{dialogMenu.value.is_pinned ? 'Открепить' : 'Закрепить'}</button><div className="context-submenu-label">Добавить в папку ›</div>{(Array.isArray(folders.data) ? folders.data : []).filter(folder => folder.account_id === dialogMenu.value.account_id).map(folder => { const active = Boolean(dialogMenu.value.folder_ids?.includes(folder.id)); return <button key={folder.id} onClick={() => setFolderMembership.mutate({ folder: folder.id, dialog: dialogMenu.value.id, enabled: !active })}>{active ? '✓ ' : ''}{folder.name}</button> })}<button onClick={() => { beginCreateFolder(dialogMenu.value.account_id); setDialogMenu(null) }}>+ Новая папка</button></>}</div></ScaledOverlay>}
    {messageMenu && <ScaledOverlay point={{ x: messageMenu.x, y: messageMenu.y }}><div className="context-menu" onClick={event => event.stopPropagation()}><button onClick={() => { setReplyTargetId(messageMenu.value.vk_message_id); setMessageMenu(null); textareaRef.current?.focus() }}>Ответить</button><button onClick={() => { setForwardTarget(messageMenu.value); setMessageMenu(null) }}>Переслать</button>{messageMenu.value.outgoing && <button onClick={() => { const text = prompt('Изменить сообщение', messageMenu.value.body)?.trim(); if (text != null) editMessage.mutate({ message: messageMenu.value, text }) }}>Редактировать</button>}<button onClick={() => navigator.clipboard?.writeText(messageMenu.value.body || '')}>Скопировать текст</button><button onClick={() => navigator.clipboard?.writeText(String(messageMenu.value.vk_message_id))}>Скопировать ID</button><button className="danger-text" onClick={() => { if (confirm('Удалить сообщение у всех, если VK это разрешит?')) deleteMessage.mutate(messageMenu.value) }}>Удалить</button></div></ScaledOverlay>}
    {forwardTarget && <ScaledViewportOverlay><div className="modal-backdrop" onClick={() => setForwardTarget(null)}><div className="forward-picker" onClick={event => event.stopPropagation()}><header><strong>Переслать сообщение</strong><button onClick={() => setForwardTarget(null)}><X size={15} /></button></header><p>Настоящая пересылка доступна в диалоги того же VK-аккаунта.</p>{forwardDialogs.length ? forwardDialogs.map(dialog => <button key={dialog.id} onClick={() => send.mutate({ dialogId: dialog.id, text: '', replyTo: undefined, forward: { peer_id: activeDialog?.peer_id, message_ids: [forwardTarget.vk_message_id] } })}>{dialog.avatar_url ? <img src={dialog.avatar_url} alt="" /> : <span>{dialog.title.slice(0,1)}</span>}<b>{dialog.title}</b></button>) : <div className="soft-empty">Других диалогов этого аккаунта нет</div>}</div></div></ScaledViewportOverlay>}
  </div>
}
