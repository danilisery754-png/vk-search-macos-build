...
export default function ScaledOverlay({
  ...
  const availableWidth = Math.max(0, viewportToLogical(window.innerWidth - padding * 2, scale))
  const availableHeight = Math.max(0, viewportToLogical(window.innerHeight - padding * 2, scale))
  const style = {
    position: 'absolute',
