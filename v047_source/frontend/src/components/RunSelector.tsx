...
export default function RunSelector({
  ...
  return <div className="run-selector-wrap">
    <div className="run-selector" ref={rootRef}>
      <button className="run-selector-trigger" type="button" aria-expanded={open} onClick={() => setOpen(value => !value)}><CalendarClock size={15} /><span>Запуски{selected ? ` · #${selected.id}` : ''}</span><ChevronDown size={14} /></button>
      {open && <div className="run-selector-panel" role="menu">
